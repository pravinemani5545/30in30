...
export async function POST(request: Request) {
  try {
    ...
    let reviewResult;
    try {
      reviewResult = await reviewCode(code, detectedLanguage, user?.id ?? "guest");
    } catch (err) {
      // Insert a failed record for history (authenticated only)
      if (!isGuest && supabase && user) {
    ...
    if (!isGuest && supabase && user) {
      ...
      return NextResponse.json({ review: saved, saved: true });
    }

    // Guest: return review without saving
    return NextResponse.json({
      review: {
        id: null,
        code_snippet: code,
